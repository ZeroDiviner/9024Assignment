...
// this function first clone(O(n)) the argument u, which is also a doubly linked list, and then remove duplicate(O(n^2)),then add elements to the cloned-list(O(n^2)),so the time complexity is O(n^2)
...
// this function remove duplicates twice(O(2n^2)), and then add common element to a new created list,so the time complexity is O(n^2)
